...
class BertLayerDNC(nn.Module):
    def __init__(self, config):
        ...
        self.memory = SparseMemory(input_size= config.hidden_size, mem_size=config.memory_size, cell_size=config.hidden_size,
         independent_linears=False, read_heads=1, sparse_reads=8, num_lists=None, index_checks=None, 
...
class BertModelDNCNoEmbedding(BertPreTrainedModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
...
class BertForMaskedLMUt(BertPreTrainedModel):
    """BERT model with the masked language modeling head.
    This module comprises the BERT model followed by the masked language modeling head.
    ...
    """
    def __init__(self, config):
        super(BertForMaskedLMUt, self).__init__(config)
        self.bert = BertModelUt(config)
        self.cls = BertOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        self.apply(self.init_bert_weights)
...
class UTafterBert(BertPreTrainedModel):
    """BERT model with UT Transformer aftwards
  
    """
    def __init__(self, config, config_ut):
        super(UTafterBert, self).__init__(config, config_ut)
        self.bert = BertModel(config)
        self.ut = BertModelUtNoEmbedding(config_ut)
        self.cls = BertOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        self.apply(self.init_bert_weights)
...
class TDNCafterBert(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, config_ut):
        super(TDNCafterBert, self).__init__(config, config_ut)
        self.bert = BertModel(config)
