...
def cuda(x, grad=False, gpu_id=-1):
  if gpu_id == -1:
    ...
    return var(x.pin_memory(), requires_grad=grad).contiguous().cuda(gpu_id, async=True)
...
def correct_cosine(a, b, dimA=2, dimB=2, normBy=2):
  """Batchwise Cosine distance
  ...
  Returns:
      Tensor -- Batchwise cosine distance (b * r * m)
  """
  a_norm = T.norm(a, normBy, dimA, keepdim=False) + δ
  b_norm = T.norm(b, normBy, dimB, keepdim=False) + δ

  x = T.bmm(a, b.transpose(1, 2)) / (
      a_norm.unsqueeze(2) * b_norm.unsqueeze(1) + δ)
  # apply_dict(locals())
  return x

def deepmindcosine(a, b, dimA=2, dimB=2, normBy=2):
  """Batchwise Cosine distance
  ...
  x = T.bmm(a, b.transpose(1, 2)) / (
      T.bmm(a_norm, b_norm.transpose(1, 2)) + δ)
  # apply_dict(locals())
  return x
