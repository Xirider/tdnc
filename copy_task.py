...
if __name__ == '__main__':
  ...
  #   input_data, target_output = generate_data(batch_size, random_length, input_size, cuda=0)

  #   if rnn.debug:
  #     output, (chx, mhx, rv), v = rnn(input_data, (None, mhx, None), reset_experience=True, pass_through_memory=True)
  #   else:
  #     output, (chx, mhx, rv) = rnn(input_data, (None, mhx, None), reset_experience=True, pass_through_memory=True)
  #   output = output[0, -1, :].sum().data.cpu().numpy()

  #   target_output = target_output[0,-1,:].sum().data.cpu().numpy()
  ...
  for i in range(int((iterations + 1) / 10)):
    ...
    # We test now the learned generalization using sequence_max_length examples
    random_length = np.random.randint(2, sequence_max_length * 10 + 1)
    input_data, target_output, loss_weights = generate_data(random_length, args.input_size)
